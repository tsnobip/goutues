...
@obj
external // These properties are part of the Fetch Standard
options: (
  ~method: method_=?, // 'GET',
  ...
  ~follow: //~signal: null,           // Pass an instance of AbortSignal to optionally abort requests
  // The following properties are node-fetch extensions
  int=?, // 20,             // maximum redirect count. 0 to not follow redirect
  ...
  ~highWaterMark: // ~agent: null,            // http(s).Agent instance or function that returns an instance (see below)
  int=?, // 16384,   // the maximum number of bytes to store in the internal buffer before ceasing to read from the underlying resource.
...
@module
external fetch: (string, ~options: options<'body>=?, unit) => Promise.t<response> = "node-fetch"
